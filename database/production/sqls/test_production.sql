CREATE DATABASE IF NOT EXISTS testdb_production;

use testdb_production;
...
BEGIN
    ...
    WHERE table_schema = 'testdb_production'
...
BEGIN
    ...
    ELSE
        SELECT CONCAT('Index ','sender ','already exists on Table ', 'testdb_production','.','email_sender');   
    END IF;
    ...
    ELSE
        SELECT CONCAT('Index ','receiver ','already exists on Table ', 'testdb_production','.','email_receiver');   
    END IF;
    ...
    ELSE
        SELECT CONCAT('Index ','subject ','already exists on Table ', 'testdb_production','.','email_subject');   
    END IF;
    ...
    ELSE
        SELECT CONCAT('Index ','content ','already exists on Table ', 'testdb_production','.','email_content');   
    END IF;
