CREATE DATABASE IF NOT EXISTS testdb_development;

use testdb_development;
...
BEGIN
    ...
    WHERE table_schema = 'testdb_development'
...
BEGIN
    ...
    ELSE
        SELECT CONCAT('Index ','sender ','already exists on Table ', 'testdb_development','.','email_sender');   
    END IF;
    ...
    ELSE
        SELECT CONCAT('Index ','receiver ','already exists on Table ', 'testdb_development','.','email_receiver');   
    END IF;
    ...
    ELSE
        SELECT CONCAT('Index ','subject ','already exists on Table ', 'testdb_development','.','email_subject');   
    END IF;
    ...
    ELSE
        SELECT CONCAT('Index ','content ','already exists on Table ', 'testdb_development','.','email_content');   
    END IF;
